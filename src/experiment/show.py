...
def show_mmlu():
    ...
    data = pd.DataFrame({
        ...
        'Accuracy': values1 + values2,
        'Model': ['base']*len(categories) + ['tunned']*len(categories)
    })

    # 绘制分组柱状图
    plt.figure(figsize=(14, 7))
    sns.barplot(x='Category', y='Accuracy', hue='Model', data=data, 
                palette={'base':'skyblue', 'tunned':'salmon'}, alpha=0.7)
...
def show_correctness():
    ...
    sns.lineplot(data=lora_list, label='tunned8 model', 
...
def show_collision():
    ...
    fail2_list = list(tunned10_2_fail.values())
    ...
    line2, = ax1.plot(tunned10_2_list, 'm-', label='tunned102 score')
    
    # 绘制柱状图（右侧Y轴）
    width = 0.4  # 柱状图宽度
    x = np.arange(len(fail1_list))
    bar1 = ax2.bar(x - width/2, fail1_list, width, alpha=0.5, label='tunned101 fail times')
    bar2 = ax2.bar(x + width/2, fail2_list, width, alpha=0.5, label='tunned102 fail times')
